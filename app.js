...
const getTimeLeft = async () => {
  const result = await db.TimeLeft.findOne({ where: {}, order: [['createdAt', 'DESC']] });
  return result ? result.timeLeft : 120 * 24 * 3600;
};

const updateTimeLeft = async (timeLeft) => {
  await db.TimeLeft.create({ timeLeft });
};
...
const startServer = async () => {
  ...
  app.post("/reset-time", async (req, res) => {
